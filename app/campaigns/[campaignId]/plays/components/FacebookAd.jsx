...
export default function FacebookAd({ adValues }) {
	...
	return (
		<div className={`${styles.p__facebook_ad_preview} p-3 border rounded-md`}>
			...
			{/* Primary Text */}
			<div>
				{adValues.Body_Text_Only__c ? (
					<p className='text-sm py-3 whitespace-pre-wrap' dangerouslySetInnerHTML={{ __html: adValues.Body_Text_Only__c }}></p>
				) : (
			...
			<div className='p-2 py-4 bg-slate-50 flex justify-between items-center'>
				<div className='flex-column basis-2/3'>
					...
					<p className='font-medium'>{adValues.Subject__c ? adValues.Subject__c : "Headline here"}</p>
				</div>
				<div className='basis-1/3 flex justify-end'>
					<button type='button' className='rounded-md bg-zinc-200 px-3 py-2 text-sm font-medium text-gray-900 shadow-sm hover:bg-zinc-300'>
						{adValues.Call_To_Action__c ? toSentenceCase(adValues.Call_To_Action__c) : "Learn More"}
					</button>
				</div>
			</div>
			{/* Social Buttons */}
